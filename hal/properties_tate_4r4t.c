...
\
    ...
    \
    /*Will probably need to be overhauled when porting*/\
    static int hdlr_tx_##ch##_get_dyn_link_latency_1(const char *data, char *ret) {       \
        int req = 0;                                                          \
        sscanf(data, "%i", &req);                                             \
        if( req == -1 ) {\
            return RETURN_SUCCESS;\
        }\
        int reg_val = read_dac_reg(uart_tx_fd[INT_TX(ch)], INT(ch), 0x303); \
        snprintf(ret, 20, "0x%i", reg_val);\
        return RETURN_SUCCESS;                                                 \
    }\
    \
    /*Will probably need to be overhauled when porting*/\
    ...
    \
    /*Will probably need to be overhauled when porting*/\
    static int hdlr_tx_##ch##_set_fix_link_lat_1(const char *data, char *ret) {       \
        int req = 0;                                                          \
        sscanf(data, "%i", &req);                                             \
        if( req == -1 ) {\
            return RETURN_SUCCESS;\
        ...
        write_dac_reg(uart_tx_fd[INT_TX(ch)], INT(ch), 0x305, val);\
        return RETURN_SUCCESS;                                                 \
    }\
    \
    /*Will probably need to be overhauled when porting*/\
    ...
    \
    /*Will probably need to be overhauled when porting*/\
    static int hdlr_tx_##ch##_set_var_link_lat_1(const char *data, char *ret) {       \
        int req = 0;                                                          \
        sscanf(data, "%i", &req);                                             \
        if( req == -1 ) {\
            return RETURN_SUCCESS;\
        ...
        write_dac_reg(uart_tx_fd[INT_TX(ch)], INT(ch), 0x307, val);\
        return RETURN_SUCCESS;                                                 \
    }\
    \
...
#define DEFINE_TX_CHANNEL(_c)                                                                                         \
    ...
    DEFINE_FILE_PROP_P("tx/" #_c "/jesd/get_dyn_link_latency_1" , hdlr_tx_##_c##_get_dyn_link_latency_1, RW, "-1", SP, #_c)\
    DEFINE_FILE_PROP_P("tx/" #_c "/jesd/set_fix_link_lat_0" , hdlr_tx_##_c##_set_fix_link_lat_0, RW, "-1", SP, #_c)\
    DEFINE_FILE_PROP_P("tx/" #_c "/jesd/set_fix_link_lat_1" , hdlr_tx_##_c##_set_fix_link_lat_1, RW, "-1", SP, #_c)\
