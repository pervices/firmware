...
static void make_prop(prop_t *prop) {
    ...
    switch (prop->type) {
        ...
        case PROP_TYPE_SYMLINK: {
            ...
            // symlink will fail if any file already exists
            int unlink_r = unlink(path);
            // errno == ENOENT indicates there is no file to delete which is expected on boot
            if(unlink_r < 0 && errno != ENOENT) {
                PRINT(ERROR, "Failed to clear old file for symlink from %s due to: %s\n", path, strerror(errno));
            }

            // Get absolute path of the target
            char dst_path[MAX_PROP_LEN];
            snprintf(dst_path, MAX_PROP_LEN, STATE_DIR "/%s", prop->symlink_target);

            // Create symlink
            int symlink_r = symlink(dst_path, path);

            if(symlink_r < 0) {
                PRINT(ERROR, "Failed to create symlink from %s to %s due to: %s\n", dst_path, path, strerror(errno));
                break;
            }
...
static void init_prop_val(prop_t *prop) {
    ...
    if (strcmp(prop->path, "save_config") == 0 ||
