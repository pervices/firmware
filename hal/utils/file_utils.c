...
int mkdir_p(const char* path) {
    ...
    // 0777 will be modified by umask to result in the default permissions
...
int touch_p(const char* path) {
    ...
    if(errno == ENOENT) {
        ...
        int dirname_r = simple_dirname(parent, path);
