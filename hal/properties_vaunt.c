...
#define AVERY_IF_RX 650000000UL
#if PRODUCT_RTM_VER == 2
    #define AVERY_IF_TX 650000000UL
#else
    #define AVERY_IF_TX 850000000UL
#endif
...
// Check if the unit is configured so all tx can handle max rate but rx C and D don't work
static int hdlr_is_full_tx(const char *data, char *ret) {
...
#define X(ch)                                                                  \
    ...
    static int hdlr_rx_##ch##_trigger_sample_counter(const char *data, char *ret) {\
        ...
        uint32_t val2;\
        uint64_t val;\
        read_hps_reg("rx" STR(ch) "13", &val2);\
        read_hps_reg("rx" STR(ch) "14", &val1);\
...
#define X(ch)                                                                  \
    ...
    static int hdlr_tx_##ch##_rf_freq_val(const char *data, char *ret) {       \
        ...
        if (PRODUCT_ID == AVERY_ID) {                                          \
            ...
            if (freq > MAX_RF_FREQ) { /*out of bounds, too high*/              \
                ...
                float n = ((float)freq - AVERY_IF_TX ) / (4 * pll.ref_freq / pll.R);\
                ...
                freq -= lmx_freq;                                              \
                if (freq > AVERY_IF_TX + 3 * pll.ref_freq / pll.R)             \
                    freq = AVERY_IF_TX + 3 * pll.ref_freq / pll.R;             \
                else if (freq < AVERY_IF_TX - 3 * pll.ref_freq / pll.R)        \
                    freq = AVERY_IF_TX - 3 * pll.ref_freq / pll.R;             \
                ...
                /* round the requested freq to the nearest multiple of phase*/ \
                /* detector frequency around the 650MHz target IF*/            \
                float n = ((float)freq - AVERY_IF_TX ) / ( pll.ref_freq / pll.R );\
                lmx_freq = round(n) * pll.ref_freq / pll.R;                    \
                /* run the pll calc algorithm */                               \
                outfreq = setFreq(&lmx_freq, &pll);                            \
                while ((pll.N < pll.n_min) && (pll.R < pll.r_max)) {           \
    ...
    static int hdlr_tx_##ch##_trigger_sample_counter(const char *data, char *ret) {\
        ...
        uint32_t val2;\
        uint64_t val;\
        read_hps_reg("tx" STR(ch) "17", &val2);\
        read_hps_reg("tx" STR(ch) "18", &val1);\
        val = (val2);\
        val = ((uint64_t)val1 << 32) | (val2);\
        snprintf(ret, MAX_PROP_LEN, "%llu\n", val);\
        return RETURN_SUCCESS; \
    ...
    static int hdlr_rx_##ch##_rf_freq_val(const char *data, char *ret) {       \
        ...
        if (PRODUCT_ID == AVERY_ID) {                                                     \
            if (freq > MAX_RF_FREQ) { /*out of bounds, too high*/              \
                ...
                float n = ((float)freq - AVERY_IF_RX ) / (4 * pll.ref_freq / pll.R);\
                lmx_freq = round(n) * pll.ref_freq / pll.R;                    \
                /* run the pll calc algorithm */                               \
                outfreq = setFreq(&lmx_freq, &pll);                            \
                while ((pll.N < pll.n_min) && (pll.R < pll.r_max)) {           \
                ...
                /* passband by up to 3x base clock.*/                          \
                freq -= lmx_freq;                                              \
                if (freq > AVERY_IF_RX + 3 * pll.ref_freq / pll.R)             \
                    freq = AVERY_IF_RX + 3 * pll.ref_freq / pll.R;             \
                else if (freq < AVERY_IF_RX - 3 * pll.ref_freq / pll.R)        \
                    freq = AVERY_IF_RX - 3 * pll.ref_freq / pll.R;             \
            } else if (freq > 6000000000) { /*Front end mid band 6GHz - 20GHz*/\
                /* select the band*/                                           \
                ...
                /* round the requested freq to the nearest multiple of phase*/ \
                /* detector frequency around the 650MHz target IF*/            \
                float n = ((float)freq - AVERY_IF_RX ) / ( pll.ref_freq / pll.R );\
                lmx_freq = round(n) * pll.ref_freq / pll.R;                    \
                /* run the pll calc algorithm */                               \
                outfreq = setFreq(&lmx_freq, &pll);                            \
                while ((pll.N < pll.n_min) && (pll.R < pll.r_max)) {           \
...
// Track current PPS polarity to avoid needing to wait for PPS detection
typedef enum {
    PPS_UNKNOWN,
    PPS_EXTERNAL,
    PPS_INTERNAL
} pps_polarity_t;
static pps_polarity_t existing_polarity = PPS_UNKNOWN;
...
static int hdlr_time_set_time_source(const char *data, char *ret) {
    ...
    if(pps_good) {
        ...
        return RETURN_SUCCESS;
    } else if(existing_polarity == PPS_INTERNAL) {
        PRINT(ERROR, "Failed to detect internal PPS\n");
        return RETURN_SUCCESS;
    }

    PRINT(ERROR, "External PPS signal not detected, falling\n");
