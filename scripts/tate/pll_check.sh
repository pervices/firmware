...
for CURR_ATTEMPT in $(seq 1 $MAX_ATTEMPTS); do
    ...
    if [[ ! -z $uart_timeout ]]; then
        echo "[ERROR][ATTEMPT $CURR_ATTEMPT]: Unable to communicate with the time board."
    else
        unlocked=$(echo $output | grep Unlocked)
        if [[ ! -z $unlocked ]]; then
            echo "[ERROR][ATTEMPT $CURR_ATTEMPT]: PLL(s) unlocked"
        else
            # 3 LMKs, each with 2 PLLs means we're expecting to see `Locked` exactly 6 times in the response. 
            # Anything else is unexpected and thus considered a failure.
            ...
            if [[ $lock_count -eq 6 ]]; then
                ...
                echo "[ERROR][ATTEMPT $CURR_ATTEMPT]: Unrecognized response from time board."
            fi
        fi
    fi
